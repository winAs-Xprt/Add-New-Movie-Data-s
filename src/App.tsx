...
function App() {
  ...
  const fetchNewMovies = async () => {
    ...
    setNewMovies(moviesData); // Update the state with the new movies from Firestore
  };

  useEffect(() => {
    fetchStaticMovies(); // Fetch static movies on component mount
    fetchNewMovies(); // Fetch new movies on component mount
  }, []);

  // Function to add a new movie
  const addMovieToState = (newMovie: Movie) => {
    setNewMovies(prevMovies => [...prevMovies, newMovie]); // Append the new movie to the new movies state
  };
